...
const rplArrayMerge = (source, ...objArr) => {
  const isRefrence = (checkItem) => checkItem && ['object', 'function'].includes(typeof checkItem);
  const cloneType = (val) => {
    if (isRefrence(val)) {
  ...
  const rplArrayFunc = (before, after) => {
...
exports.rplArrayMerge = rplArrayMerge;
